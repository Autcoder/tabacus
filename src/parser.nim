...
proc isOperator(k: Kind): bool =
  k in {tkPlus, tkMinus, tkUnaryMinus, tkTimes, tkDiv, tkPower, tkMod}
  ...
  for token in tokens:
    ...
    of tkRPar:
      ...
      if not foundLeftParen:
        raise newException(
          ValueError, "Mismatched parentheses: unexpected closing parenthesis"
        )
      ...
      if isOperator(token.kind):
        while stack.len > 0 and isOperator(stack[^1].kind):
          ...
          if precedence(stack[^1].kind) > precedence(token.kind) or (
            precedence(stack[^1].kind) == precedence(token.kind) and
            not isRightAssociative(token.kind)
          ):
            output.add(stack.pop())
          else:
            break
